...
QHexView::QHexView(QWidget *parent) : QAbstractScrollArea(parent),
		address_color_(Qt::red), even_word_(Qt::blue),
...
QMenu *QHexView::createStandardContextMenu() {
	...
	menu->addAction(tr("设置字体"), this, SLOT(mnuSetFont()));
	menu->addSeparator();
	add_toggle_action_to_menu(menu, tr("显示地址列"), show_address_, this, SLOT(setShowAddress(bool)));
	add_toggle_action_to_menu(menu, tr("显示十六进制列"), show_hex_, this, SLOT(setShowHexDump(bool)));
	add_toggle_action_to_menu(menu, tr("显示ASCII列"), show_ascii_, this, SLOT(setShowAsciiDump(bool)));
	...
	QMenu *const wordMenu = new QMenu(tr("设置字宽"), menu);
	QAction *const a1 = add_toggle_action_to_menu(wordMenu, tr("1 字节"), word_width_ == 1, wordWidthMapper, SLOT(map()));
	QAction *const a2 = add_toggle_action_to_menu(wordMenu, tr("2 字节"), word_width_ == 2, wordWidthMapper, SLOT(map()));
	QAction *const a3 = add_toggle_action_to_menu(wordMenu, tr("4 字节"), word_width_ == 4, wordWidthMapper, SLOT(map()));
	QAction *const a4 = add_toggle_action_to_menu(wordMenu, tr("8 字节"), word_width_ == 8, wordWidthMapper, SLOT(map()));
	...
	QMenu *const rowMenu = new QMenu(tr("设置行宽"), menu);
	QAction *const a5 = add_toggle_action_to_menu(rowMenu, tr("1 字"), row_width_ == 1, rowWidthMapper, SLOT(map()));
	QAction *const a6 = add_toggle_action_to_menu(rowMenu, tr("2 字"), row_width_ == 2, rowWidthMapper, SLOT(map()));
	QAction *const a7 = add_toggle_action_to_menu(rowMenu, tr("4 字"), row_width_ == 4, rowWidthMapper, SLOT(map()));
	QAction *const a8 = add_toggle_action_to_menu(rowMenu, tr("8 字"), row_width_ == 8, rowWidthMapper, SLOT(map()));
	QAction *const a9 = add_toggle_action_to_menu(rowMenu, tr("16 字"), row_width_ == 16, rowWidthMapper, SLOT(map()));
	...
	menu->addSeparator();
	menu->addAction(tr("复制选中"), this, SLOT(mnuCopy()));
	menu->addAction(tr("编辑"), [this]
	{
		emit editHex(selection_start_);
	});

	menu->addAction(tr("转到地址"), [this]
	{
		QDialog dlg;
		...
		scrollTo(edit->text().toULongLong(nullptr, 16));
	});
...
void QHexView::scrollTo(address_t offset) {
	if (offset < address_offset_ || offset >= (address_offset_ + dataSize()))
	{
		...
		buffer->resize(size);
		if (!debugCore->readMemory(start, buffer->data(), size))
		{
			log("readMemory failed in QHexView::scrollTo", LogType::Error);
			return;
		}

		setAddressOffset(start);
		...
		data_ = new QBuffer(buffer, this);
		data_->open(QBuffer::ReadOnly);
...
void QHexView::setDebugCore(std::shared_ptr<DebugCore> debugCore)
{
